...
//    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        let titleString = customSegmentProperties.titleArray[indexPath.row]
        let width: CGFloat = self.getWidthOfText(titleString as! String) + 10
        let itemSize = CGSize(width: width, height: CGFloat(self.frame.size.height))
        return itemSize
    ...
    func getWidthOfText(_ text: String) -> CGFloat {
        let frameSize = CGSize(width: CGFloat(300), height: CGFloat(50))
        let font = UIFont.systemFont(ofSize: CGFloat(12))
        let idealFrame = text.boundingRect(with: frameSize, options: .usesLineFragmentOrigin, attributes: [NSFontAttributeName: font], context: nil)
